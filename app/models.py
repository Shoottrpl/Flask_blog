...
class User(UserMixin, db.Model):
    ...
    notifications: db.WriteOnlyMapped['Notification'] = db.relationship(
        back_populates='user')

    tasks: db.WriteOnlyMapped['Task'] = db.relationship(
        back_populates='user')
...
class Message(db.Model):
    ...
    text: db.Mapped[str] = db.mapped_column(sa.String(140))
...
class Notification(db.Model):
    id: db.Mapped[int] = db.mapped_column(primary_key=True)
    name: db.Mapped[str] = db.mapped_column(sa.String(128))
    user_id: db.Mapped[int] = db.mapped_column(sa.ForeignKey(User.id))
    timestamp: db.Mapped[float] = db.mapped_column(default=time)
    payload_json: db.Mapped[str] = db.mapped_column(sa.Text)
...
class Task(db.Model):
    id: db.Mapped[str] = db.mapped_column(sa.String(36), primary_key=True)
    name: db.Mapped[str] = db.mapped_column(sa.String(128))
    description: db.Mapped[str] = db.mapped_column(sa.String(128))
    user_id: db.Mapped[int] = db.mapped_column(sa.ForeignKey(User.id))
    complete: db.Mapped[bool] = db.mapped_column(default=False)
