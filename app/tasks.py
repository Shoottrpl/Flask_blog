...
def export_posts(user_id):
    try:
        ...
        for post in db.session(user.posts.select().order_by(Post.timestamp.asc())):
        ...
        send_mail(
            '[Testsite] ваши публикации', sender=app.config['ADMIN'][0], recipients=[user.email],
            text_body=render_template('email/export_posts.txt', user=user),
            html_body=render_template('emai/export_posts.html', user=user),
